...
class CPUForgetMult(torch.nn.Module):
    ...
    def forward(self, f, x, wids, zero_wid, zero_vec, hidden_init=None):
        ...
        prev_h = hidden_init
        wids = wids.chunk(wids.size(1), dim=1)
        #print(wids)
        for i, h in enumerate((f * x).split(1, dim=0)):
            #print((wids[i] != zero_wid).view(1, -1, 1).float())
            #print(((1 - forgets[i]) * prev_h))
...
if __name__ == '__main__':
    ...
    #wids = Variable(torch.LongTensor([[0, 2, 42, 3]]).cuda())

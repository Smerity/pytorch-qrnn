...
class QRNNLayer(nn.Module):
    ...
    def __init__(self, input_size, hidden_size=None, save_prev_x=False, zoneout=0, window=1, output_gate=True, use_cuda=True, layer_norm=False, z_activation=torch.nn.functional.tanh, zeroed=False):
        ...
        if self.zeroed:
            self.start_vec = Variable(torch.zeros(self.hidden_size).cuda(), requires_grad=True)
    ...
    def forward(self, X, hidden=None, wids=None, zero_wid=None):
        ...
        if wids is not None:
            C = ZeroedForgetMult()(F, Z, wids, zero_wid, self.start_vec, hidden_init=hidden, use_cuda=self.use_cuda)
        else:
